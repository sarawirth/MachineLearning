...
frequent_itemsets <- apriori(binary_matrix, parameter = list(support = 0.3, 
...
quality(frequent_itemsets[1:15])


rules <- apriori(binary_matrix, 
                 parameter = list(supp = (1), 
                                  conf = 1,
                                  minlen = 3,
                                  maxlen = 6),
...
filtered_rules <- subset(rules, support < 1)
inspect(filtered_rules)
nrow(filtered_rules)

sorted_s <- sort(rules, by = "support", decreasing = FALSE)
sorted_c <- sort(rules, by = "confidence", decreasing = TRUE)
sorted_l <- head(sort(rules, by = "lift", decreasing = TRUE),20)

inspect(sorted_s[1:15])
inspect(sorted_c[1:10])
inspect(sorted_l[1:10])
...
# Convert the rules to a data fram

subrules <- head(sorted_l,10)
plot(subrules)
